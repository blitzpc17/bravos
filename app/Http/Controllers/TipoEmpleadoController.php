...
class TipoEmpleadoController extends Controller
{
    ...
    public function save(Request $r)
    {
        try{
            ...
            {                
                ...
                $result = TipoEmpleado::create($arrayData);
                if(isset($result->id)){
            ...
            }else if($r->op=='M')
            {
                ...
                $result = TipoEmpleado::Modificar($data ,$r->id);
                if($result==1){
    ...
    public function ListarRegistros(){
        return response()->json(["data"=>TipoEmpleado::all()]);
    }
    public function ObtenerRegistro($id){
        return TipoEmpleado::where('id', $id)->first();

    }
    public function EliminarRegistro($id){
        try{
            $result = TipoEmpleado::where('id', $id)->delete();
            if($result == 1){
