...
use \App\Models\TipoServicioBravos;
...
class TipoServicioController extends Controller
{
    ...
    public function index(){
        $user = Auth::user();
        return view('Backend.sistema.tipoServicio', compact('user'));
    }
    ...
    public function save(Request $r)
    {
        try{
            ...
            {                
                ...
                $result = TipoServicioBravos::create($arrayData);
                if(isset($result->id)){
            ...
            }else if($r->op=='M')
            {
                ...
                $result = TipoServicioBravos::Modificar($data ,$r->id);
                if($result==1){
    ...
    public function ListarRegistros(){
        return response()->json(["data"=>TipoServicioBravos::all()]);
    }
    public function ObtenerRegistro($id){
        return TipoServicioBravos::where('id', $id)->first();

    }
    public function EliminarRegistro($id){
        try{
            $result = TipoServicioBravos::where('id', $id)->delete();
            if($result == 1){
