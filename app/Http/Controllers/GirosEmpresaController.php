...
use \App\Models\GirosEmpresa;
use \App\Models\Utilidades;
use Validator;
use Auth;
use Illuminate\Support\Facades\Log;

class GirosEmpresaController extends Controller
{
    ...
    public function index(){
        $user = Auth::user();
        return view('Backend.sistema.girosEmpresa', compact('user'));
    }
    ...
    public function save(Request $r)
    {
        try{
            ...
            {                
                ...
                $result = GirosEmpresa::create($arrayData);
                if(isset($result->id)){
            ...
            }else if($r->op=='M')
            {
                ...
                $result = GirosEmpresa::Modificar($data ,$r->id);
                if($result==1){
    ...
    public function ListarRegistros(){
        return response()->json(["data"=>GirosEmpresa::all()]);
    }
    public function ObtenerRegistro($id){
        return GirosEmpresa::where('id', $id)->first();

    }
    public function EliminarRegistro($id){
        try{
            $result = GirosEmpresa::where('id', $id)->delete();
            if($result == 1){
